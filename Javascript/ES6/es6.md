...
- 历史版本
    ...
    ![](C:\Users\Administrator\Desktop\webpack和grunt以及gulp的对比.png)
...
- let 和 const
    ...
    - let 是变量，可以修改
    - const 是常量，不能修改
- const修饰的标识符必须赋值`const name;  // 错误`

常量的含义是指向的对象不能修改（即保存的指向对象的地址是固定的，不可变的），但是可以改变对象内部的属性
...
obj = {}	//错误， 指向一个新的对象其地址改变了，而常量不能改变其保存的地址
// 可以修改对象内部的属性
obj.name = '万人迷',
obj.age = '23'
```
...
- 参数扩展／展开 `...args`(剩余参数rest parameter，args名称随意取)
    - 收集剩余的参数，必须放到最后一个参数位置
...
3. `Number.parseInt(str)`：将字符串转为对应的数值
...
var result = arr.reduce(function (tmp, item, index) {
    //tmp 上次结果（之前累加的），item当前数，index迭代次数（次数1开始）
    console.log(tmp, item, index)	//
    return tmp + item
})
console.log(result) 	//1+3+5+7=16

// 求平均
var result = arr.reduce(function (tmp, item, index) {
    if (index != arr.length - 1) { // 不是最后一次求和
        return tmp + item
    } else {	//最后一次
...
``` javascript
let result = Array.of(1, 'abc', true)
console.log(result)	// [1, abc, true]
```

3. `find(function(value, index, arr) {return true})`：找出第一个满足条件返回 true 的元素

```javascript
let arr = [2, 4, 3, 5, 4, 1, 6]
...
console.log(result)	//3		[5对应的下标]
```

4. `findIndex(function(value, index, arr) {return true})`：找出第一个满足条件返回 true 的元素**下标**
...
```javascript
console.log(NaN === NaN)	//false , NaN和任何数据都不相等包括自己，六亲不认
console.log(0 === -0)	//true

console.log(Object.is(NaN, NaN))	// true
// 实质是转化为了字符串形式判断 Object.is('NaN', 'NaN')
...
console.log(obj2)	//{name: 'wrm', age: 22}
```
...
- JSON 对象
    - JSON 对象是 JavaScript 的**原生对象**，用来处理 JSON 格式数据，有两个静态方法
...
- generator 生成器函数
    - 普通函数特点：一路走到底
    - generator函数，**中间可以停**，到哪停呢，与yield 配合，交出执行权
    - yield 有 放弃、退让、退位的意思
    - 需要调用next()方法启动执行，需要遇到 yield 停, 踹一脚走一步
    - generator函数前面加一个 `*` 两边可以有空格，或靠近函数或`function`，不能写成箭头函数形式
    - 背后实际生成多个小函数，实现走走停停
...
//generator函数 调用不会立即执行，而是创建了一个generator对象
let genObj = show2()
//需要调用generator对象的next方法才能执行
genObj.next() // 1
genObj.next() // 2
...
// yield 传参
var gen = show()
//next()中传的参数就会来到yield中，作为yield的参数
gen.next(10) // 1 第一次执行到yield，但没有执行赋值（a=yield未执行），即第一个next传参是没办法传给yield的，如果想传，通过show(参数)正常传即可
gen.next(20) // 2 和 20
```
...
- async await
    - 和 generator yield 类似 ，generator的语法糖
