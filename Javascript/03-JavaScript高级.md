...
原型对象的所有属性和方法，都会被构造函数的实例继承。

可以将实例对象需要共享的属性和方法直接定义在 `prototype` 对象上。
...
作用：**改变this指向**，默认是window，第一个参数thisArg传谁，this就改为指向谁
...
一个函数能够访问和操作另一个函数作用域中的变量时，就构成了一个闭包。
...
闭包是一种使函数够访问其他函数的作用域中（函数内部）变量的语法机制，
