...
样式的代码是在style的标签中设置（即写在css样式表中），通过style获取不到。此时用offset(获取的是数字类型)

样式的代码是在style的属性中设置（即写在行内式style属性中），通过style可以获取
...
API（Application Programming Interface,应用程序编程接口）是一些预先定义的函数，（**理解为方法和工具**）目的是提供应用程序与开发人员基于某软件或硬件得以访问一组例程的能力，而又无需访问源码，或理解内部工作机制的细节。
...
// 取消定时器的执行，虽然只执行一次，但是还需要清理掉定时器，因为其占用内存空间
...
location.past//该网页的相对路径
location.port//端口号
location.protocol//协议名称
location.search//搜索的名称
```
...
location.replace("url")//没有历史纪录，不能返回
...
console.log(box.offsetWidth);//有边框的距离
...
元素内实际内容的宽高（没有边框），如果没有内容

```javascript
var box = document.getElementById('box');
console.log(box.scrollLeft)
console.log(box.scrollTop)//向上卷去出去的距离，滚动条向上隐藏的距离
